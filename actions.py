...
def follow_user(**kwargs):
    ...
    return (already_followed,private_account,blocked)
...
def like_latest_posts(**kwargs):
    ...
    for i in range(num_posts):
        ...
        posts = wait.until(EC.presence_of_all_elements_located((By.CLASS_NAME, "v1Nh3")))
        time.sleep(random.randint(1, 3))

        # Open post
        post_url = posts[i].find_element_by_css_selector("a").get_attribute("href")
        driver.get(post_url)

        # Get like button
        like_button = wait.until(EC.presence_of_element_located((By.CSS_SELECTOR, "button svg")))
        time.sleep(random.randint(1, 3))
